  ...
  <div class="content-wrapper">
    <div class="card card-warning">
      <div class="card-header d-flex justify-content-start">
        <h3 class="card-title">ثبت غذای جدید</h3>
      </div>
      <!-- /.card-header -->
      <div class="card-body">
        ...
        <form role="form" action="{{ route('foods.store') }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <select name="food_type_id" class="form-control">
              @foreach(\App\Models\FoodType::where('active' , 1)->get() as $foodType)
                <option value="{{ $foodType->id }}" {{ (old('food_type_id ') && old('food_type_id ') == $foodType->id) ? 'selected' : '' }}>{{ $foodType->title }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div class="form-group">
            <label>تاریخچه</label>
            <textarea name="description" class="form-control" rows="3" placeholder="تاریخچه">{{ old('description') }}</textarea>
          </div>
